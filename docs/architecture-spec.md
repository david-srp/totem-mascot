...
Totem 是一个 Cloudflare-hosted ZooClaw Managed Agents 样板。每个应用用户绑定一个 ZooClaw
...
| 资源 | 作用 |
|---|---|
| ZooClaw org Skill | 所有 User Agent 共同跟随的 Skill |
| App Worker | 静态站和 `/api/*` |
...
这些资源都属于当前 Builder 的 ZooClaw organization 或 Cloudflare account。仓库不提供共享资源，
...
`identity_hash` 根据 `AGENTS.md` 的真实内容计算，用于 Persona lazy sync。ZooClaw Session、事件和
artifact 仍以 ZooClaw 为 source of truth，不复制到 D1。
...
第一次执行在当前 ZooClaw key 所属 organization 创建 Skill。以后调用 `uploadSkillVersion` 发布
新版本。
...
2. hash 与 D1 相同，不调用 ZooClaw；
3. hash 不同，执行 `updateAgent({ persona })`；
4. ZooClaw 成功后更新 D1 hash。
...
User Agent unpinned 跟随 org Skill 的 `latest`。版本分发由 ZooClaw Engine 负责，D1 只保存 user
与 Agent 的 binding。
</details>
...
D1 保存该实例用户的 identity 和 Agent binding。不同 Builder 的用户和 ZooClaw organization 相互
独立，不能共享这张表。
</details>
