...
内部环境：<https://totem.impo.ai>（Cloudflare Access，只允许 `@srp.one`）
...
# .env.local 填 Identity Worker URL、service token 和 Access 配置

node --env-file=.env scripts/probe.mjs          # 验证密钥可用
node --env-file=.env scripts/upload-skill.mjs   # 首次创建 Skill；之后发布新版本
...
部署：`npm run deploy`。Cloudflare Access、D1、Workers 和生产 secrets 见
...
没有固定的 agent id。Cloudflare Access 验证 `@srp.one` 邮箱，D1 把 Access identity
